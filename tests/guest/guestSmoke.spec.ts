...
test.describe('Homepage & search filters', () => {
  test('open homepage and apply search', async ({ page }) => {
    ...
    // Check in slider our stock filter is applied
    await page.getByRole('link', { name: 'Curtainsiders' }).click();
    await expect(page).toHaveURL(/\/en-gb\/used\/trailer\/curtainsider-trailer\/?$/);
    await expect(page.getByTitle('Buy a used curtainsider semi-trailer')).toBeVisible();
